...
//            InputStream is = in;
//        	System.out.println("first inputstream");
//        	try {
//        		int i = -1;
//                for (;;) {
//                	i++;
...
//                    //System.out.printf("%02X ", byteToHexString((byte)readChar));
//                    System.out.printf("%02X ", (byte)readChar);
//                    if(i==0x500) break;
//                }
//            } finally {
//            	
//            }
        ...
        private boolean readUntil(InputStream in, byte[] b, int off, int len) throws IOException {
            System.out.printf("\nreadUtill(in, b, %08X, %d)\n", off, len);
            
//            if(flg) {
//            	System.out.println("InputStreamの中身");
//            	InputStream is = in;
//            	try {
//            		int i = -1;
//                    for (;;) {
...
//            }
            ...
            while (readCount < len) {
            	...
            	readLen = in.read(b, off + readCount, len - readCount);
            	if (readLen < 0) {
                    return false;
                }
                readCount += readLen;
        ...
        public String bytesToHexString(final byte[] argBytes) {
            final StringBuffer buffer = new StringBuffer();
            for (int byteIndex = 0; byteIndex < argBytes.length; byteIndex++) {
                if (byteIndex != 0) {
                    // 2bytes目以降には半角空白を区切り文字として付与。
                    buffer.append(' ');
                }
        ...
        public String byteToHexString(final byte argByte) {
        ...
        public byte[] loadStringBytes(int addr) {
            int i = addr;
            while (mem[i++] != 0) {}
            System.out.println("loadStringBytes");
